...
class BigNum(object):
  ...
  def slow_mul(self, other):
    long_int = long(str(self), 16)
    long_int_other = long(str(other), 16)

    '''
    Slow method for multiplying two numbers w/ good constant factors.
    '''

    return BigNum.from_hex(hex(long_int*long_int_other).rstrip("L").lstrip("0x"))
  ...
  def slow_divmod(self, other):
    ...
    '''
    return (BigNum.from_hex(hex(long_int//long_int_other).rstrip("L").lstrip("0x")),
            BigNum.from_hex(hex(long_int % long_int_other).rstrip("L").lstrip("0x")))
