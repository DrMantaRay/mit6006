...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        explored_dict = {source: 0}
        parent_dict = {source: (None, 0)}

        for node in source.adj:
            explore_queue.insert(NodeDistancePair(node, weight(node, source)))
            parent_dict[node] = (source, weight(node, source))
            num_nodes +=1

        while True:
            cur_node = explore_queue.extract_min().node

            if cur_node in explored_dict:
                continue
            explored_dict[cur_node] = explored_dict[parent_dict[cur_node][0]] + weight(cur_node, parent_dict[cur_node][0])
            if cur_node == destination:
                break
            for node in cur_node.adj:
                if node in explored_dict:
                    continue
                elif node in parent_dict:
                    if parent_dict[node][1] > weight(cur_node, node) + explored_dict[cur_node]:
                        parent_dict[node] = (cur_node, weight(cur_node, node) + explored_dict[cur_node])
                        explore_queue.insert(NodeDistancePair(node, weight(cur_node, node) + explored_dict[cur_node]))
                else:
                    explore_queue.insert(NodeDistancePair(node, weight(node, cur_node) + explored_dict[cur_node]))
                    parent_dict[node] = (cur_node, weight(node, cur_node) + explored_dict[cur_node])
                    num_nodes +=1

        node_list = []
        while parent_dict[cur_node][0] is not None:
            node_list.append(cur_node)
            cur_node = parent_dict[cur_node][0]
