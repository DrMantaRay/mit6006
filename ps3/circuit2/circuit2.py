...
class CrossVerifier(object):
  ...
  def _events_from_layer(self, layer):
    ...
    right_edge = max([wire.x2 for wire in layer.wires.values()])
    for wire in layer.wires.values():
      if wire.is_horizontal():
        self.events.append([left_edge, 0, wire.object_id, 'add', wire])
        self.events.append([right_edge, 2, wire.object_id, 'delete', wire])
      else:
    ...
    for event in self.events:
      ...
      if event_type == 'add':
        ...
        if count_only:
          ...
          for wire_cross in cross_wires:
            result.add_crossing(wire_cross.wire, wire)
